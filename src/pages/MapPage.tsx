...
import { MapPin, Filter, MessageCircle, Search, DollarSign, Map, X, ExternalLink } from 'lucide-react';
...
const MapPage = () => {
  ...
  const applyFilters = () => {
    ...
    // Budget filter
    filtered = filtered.filter(req => 
      (req.budget_min ?? 0) >= budgetRange[0] && (req.budget_max ?? 10000) <= budgetRange[1]
    );
  ...
  const initializeMap = async () => {
    ...
    try {
      ...
      if (!(window as any).google || !(window as any).google.maps) {
        const loader = new Loader({ apiKey, version: 'weekly', libraries: libraries as any });
        await loader.load();
      }
                ...
                <CardContent className="p-6">
                  <div className="space-y-4">
                    ...
                    {selectedRequest.product_link && (
                      <div className="mb-2">
                        <Button 
                          variant="outline"
                          className="w-full"
                          onClick={() => window.open(selectedRequest.product_link!, '_blank')}
                        >
                    ...
                    <div className="flex items-center gap-2">
                      <Button 
                        className="flex-1"
                        onClick={() => handleJoinRequest(selectedRequest.id)}
                        disabled={joiningRequest === selectedRequest.id}
                      >
                        {joiningRequest === selectedRequest.id ? 'Joining...' : 'Join Request'}
                        <span className="ml-2 text-xs bg-white/20 px-2 py-1 rounded">
                          1/{(selectedRequest as any).people_required || 2}
                        </span>
                      </Button>
                      <Button
                        variant="outline"
                        ...
                        onClick={() => navigateToSmartChat(selectedRequest.id, navigate)}
                      >
