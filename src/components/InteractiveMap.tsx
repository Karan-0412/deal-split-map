...
const InteractiveMap: React.FC<InteractiveMapProps> = ({
  ...
  useEffect(() => {
    (window as any).handleJoinRequest = () => {
      // Hook for external join logic
    };

        (window as any).handleChatRequest = async (requestId: string) => {
          // Hook for chat navigation
          if (isNavigating) return; // Prevent multiple clicks
          setIsNavigating(true);
          try {
            await navigateToSmartChat(requestId, navigate);
          } finally {
    ...
    const initMap = async () => {
      try {
        const apiKey = 'AIzaSyA27ZFwShXNiCI3Hso1tFvGI6Hp3dLsMAc';
      ...
      const infoWindow = new google.maps.InfoWindow({
        content: `
          <div style="padding:14px;min-width:230px;font-family:Inter, system-ui, sans-serif;">
            ...
            ${(request as any).product_link ? `
            <div style="margin-bottom:10px;">
              <a href="${(request as any).product_link}" target="_blank" rel="noopener noreferrer" style="display:flex;align-items:center;justify-content:center;gap:8px;background:#ffffff;color:#374151;border:1.5px solid #e5e7eb;border-radius:8px;padding:10px 12px;font-weight:600;cursor:pointer;text-decoration:none;transition:all 0.2s">
            ...
            ` : ''}
            <div style="${(request as any).product_link ? 'margin-top:6px;' : 'margin-top:12px;'} display:flex;gap:8px">
              <button onclick="window.handleJoinRequest('${request.id}')" style="flex:1;background:${categoryColor};color:#fff;border-radius:8px;padding:10px 12px;border:none;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px">
                Join Request
                <span style="background:rgba(255,255,255,0.2);padding:2px 6px;border-radius:4px;font-size:11px">1/${(request as any).people_required || 2}</span>
              </button>
              <button onclick="window.handleChatRequest('${request.id}')" style="background:#f3f4f6;color:#374151;border-radius:8px;padding:10px 12px;border:none;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center" title="Chat">
                💬
              </button>
            </div>
          </div>
